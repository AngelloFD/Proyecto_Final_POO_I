...
class Productosvector
{
	...
	public:
		...
		void add( productos obj ) // A�ade los objectos a una casilla del vector dinamico
		{
			vectorproductos.push_back(obj);
		}
		
		productos get( int pos ) //entregar el objeto actual, seg�n la posicion pasada como par�metro
		{
		...
		void cargarDatosDelArchivoAlVector()
		{
			try
			{
				...
				fstream archivopro;
				archivopro.open("productos.csv", ios::in);													//ios::in seg�n documentaci�n es para extraer y leer data
				if (archivopro.is_open())
				{
					
					while (!archivopro.eof()) 																//eof = end of the file, "!" significa lo contrario a lo escrito ah� (~)
					{
		...
		productos busquedaBinaria(int cod_a_buscar)
		{
			...
			{
				cout<<"NO encontrado!!! - Defaulteando al �ltimo!"<<endl;
			}
